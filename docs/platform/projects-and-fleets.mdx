...
When using any time-range property to create a device set (for example, "Last
Seen" or "Historical Data" filters), the "Start date" and "End date" values are
stored into the Device Set as relative to the time of the query, as depicted
below:

import DeviceSetSvg from "/img/docs/platform/device_search_sliding.excalidraw.svg";
