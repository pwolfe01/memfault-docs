...
| -------- | ---------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Android  | Monitoring | Only periodic device check-ins                     | [Batterystats](/docs/android/android-builtin-metrics#battery-and-related-metrics) and [Custom Metrics](/docs/android/android-custom-metrics) are collected every 2h                                                                                                                                                                                                           |
| Android  | Debugging  | No crash collection                                | [Crash reports (via Bug Reports or Caliper)](/docs/android/android-bort#collection-types---bug-reports-and-caliper), [reboot reasons](/docs/android/android-reboot-events) and [Custom Events](/docs/android/custom-events) are collected                                                                                                                                     |
| Android  | Logging    | No logs, except if they are part of a crash report | Log collection and upload in the case of an overlap with a trace event (see details [here](/docs/android/android-logging)). Logs that have been already collected on the device and not yet sent will also be uploaded as soon as the device receives the logging resolution change to `On`. For details, see [Log Collection](/docs/platform/fleet-sampling#log-collection). |
...
up, it will continue sending new logs until the resolution is set to `Off`.
