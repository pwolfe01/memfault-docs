...
Sampling Aspects on or off. See the differences between the aspects and their respective
states in the table:

| Aspects/Resolutions | Platform | Off | On |
| --- | --- | --- | --- |
| Monitoring | Android | No hourly heartbeats | Batterystats metrics are uploaded every 2h |
| Debugging | Android | No crash collection | Crash report collection (via Bug Reports or Caliper), reboot reasons |
| Logging | Android | No logs, except if they are part of a crash report | Log collection and upload in the case of an overlap with a trace event (see details here). <br /> (Only on Bort SDK, see [Log Collection](/docs/platform/fleet-sampling#log-collection)) Logs that have been already collected on the device and not yet sent will also be uploaded as soon as the device receives the logging resolution change to `On`. |
...
select _Fleet Sampling Resolutions_ and click on the edit icon <DeviceDetailEditButton /> next to the resolution value.
...
message will be shown at the top of the page when changing the sampling resolution of a
device to `On`:
...
[via API](https://api-docs.memfault.com/#f4e2584a-36f2-4184-82ec-3b9d0108588c) to mark devices you want to update
