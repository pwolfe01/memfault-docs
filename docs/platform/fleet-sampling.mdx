...
| --- | --- | --- | --- |
| Monitoring | Android | No hourly heartbeats | [Batterystats](/docs/android/android-builtin-metrics#battery-and-related-metrics) and [custom metrics](/docs/android/android-custom-metrics) are uploaded every 2h |
| Debugging | Android | No crash collection | Crash report collection (via Bug Reports or Caliper), reboot reasons |
| Logging | Android | No logs, except if they are part of a crash report | Log collection and upload in the case of an overlap with a trace event (see details [here](/docs/android/android-logging)). <br /> Logs that have been already collected on the device and not yet sent will also be uploaded as soon as the device receives the logging resolution change to `On`. For details, see [Log Collection](/docs/platform/fleet-sampling#log-collection).|
| Monitoring | MCU | No hourly heartbeats | Today’s hourly heartbeats |
| Debugging | MCU | No coredump, trace, reboot reason, CDR collection | Coredumps, traces, reboot reasons, CDR |
| Logging | MCU | No logs, except if they are part of a crash report | Regular uploads of logs|
