...
| Logging             | No logs, except if they are part of a crash report | Log collection and upload in the case of an overlap with a trace event (see details [here](/docs/android/android-logging/)). <br /> (Only on Bort SDK, see [Log Collection](/docs/platform/fleet-sampling#log-collection)) Logs that have been already collected on the device and not yet sent will also be uploaded as soon as the device receives the logging resolution change to `On` |
...
select _Fleet Sampling Resolutions_ and click on the edit icon

<DeviceDetailEditButton /> next to the resolution value.
...
Even if a device has not been sending any crash reports or monitoring data, Log Collection feature allows you to retrieve past logs from devices when the Logging resolution is set to `On`. Upon receiving the change of the Logging resolution to `On`, Device will send all previously stored logs and once caught up, it will continue sending new logs (See[Android Logging](/docs/android/android-logging) for more details). This is helpful when debugging issues on devices and find the need of previous logs for more insights. 

Logs are kept on the device as long as the storage limit permits. However, this limit is only configurable by Memfault. Please contact support@memfault.com for updating the configuration. 
...
and perform a bulk assignment searching for matching devices via the frontend as
described above.
