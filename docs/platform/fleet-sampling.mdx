...
correctly. This is enabled for fleet sampling projects by default. 
...
and download, be considered as active devices (which also contributes to the Memfault dashboard) and visible under
...
select _Fleet Sampling Resolutions_ and click on the edit icon <DeviceDetailEditButton /> next to the
resolution value.

![fleet_sampling_single_device.png](/img/docs/platform/fleet-sampling-single-device.png)
_Updating the sampling resolutions of a single device_
...
- Never reported: Device hasn’t reported any config state yet and is most
...
_Hitting the quota limit when updating sampling resolution of a single
device_
...
_Quotas and usage information_
...
updated all at once in bulk. Having such a visibility is also important before rolling
out new software versions to be able to proactively monitor the potential
negative effects of the roll out.
...
_Turning on all aspects of all devices with software version 1.0.0 and
...
_Limiting the number of devices with software version 1.0.0 that will
be updated_
...
This conveys that in the context of the screenshot above, the devices to
...
Using [Memfault’s REST API](https://api-docs.memfault.com/#2634f3fb-d137-42a8-b093-e05d420b8932) the devices belonging to a project can be listed
together with their aspect resolutions. However, there is currently no API to set the Fleet Sampling resolution programmatically. Instead, configure [Device Attributes](https://docs.memfault.com/docs/platform/metrics/#attributes) [via API](https://api-docs.memfault.com/#f4e2584a-36f2-4184-82ec-3b9d0108588c) and perform a bulk assignment searching for matching devices via the frontend as described above.
