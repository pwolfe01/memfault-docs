...
from each one of your devices. The fleet sampling feature will help keeping the
costs low by collecting diagnostics and performance data only from a subset of
your fleet but at the same time provide enough insights to understand issues
happening across your fleet.
...
Each device in the fleet has 3 aspects that represent different core features
offered by Memfault. See the differences between the aspects and their
