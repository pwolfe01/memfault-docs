...
See
[Debugging Android Battery Life](/docs/best-practices/android-battery-debugging)
...
// Called on a charger connect/disconnect event
static void battery_charger_connected_event(void) {
...
static void prv_metrics_heartbeat_record_battery_info(void) {
    ...
    //
    // This will be negative if the battery charge level increased, or on the
    // first recording, where the previous charge level is 0.
    const int32_t battery_charge_drop =
        s_battery_charge_level_previous - battery_charge_level;
    // Only record the battery drop value if a charger was not connected during
    // the interval and the computed drop is positive
    const bool report_battery_drop =
        (!s_battery_charger_event_during_interval && battery_charge_drop >= 0);

    // Store the current charge level for the next heartbeat interval
    s_battery_charge_level_previous = battery_charge_level;
    // Reset the charger connected tracking for the next heartbeat interval
    s_battery_charger_event_during_interval = battery_get_charger_connected();

    if (report_battery_drop) {
        memfault_metrics_heartbeat_set_signed(
            MEMFAULT_METRICS_KEY(Battery_ChargeLevelDrop), battery_charge_drop);
    }

    // Also record the current battery charge level
    if (battery_charge_level > 0) {
        memfault_metrics_heartbeat_set_signed(
            MEMFAULT_METRICS_KEY(Battery_ChargeLevel), battery_charge_level);
    }
}
```
...
static void prv_metrics_heartbeat_record_battery_info(void) {
    ...
    if (report_battery_drop) {
        memfault_metrics_heartbeat_set_signed(
          ...
          // Make sure to set up the range & ingress filtering to ignore -1 in
          // the Memfault service
          memfault_metrics_heartbeat_set_signed(
