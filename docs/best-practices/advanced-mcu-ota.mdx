...
## Multiple OTA "Flavors"

<!--
  - different OTA payloads for different device configurations, even with the same hardware
  - eg "algorithm a" vs "algorithm b"
  - can be done with cohorts to deploy targeted OTA payloads, eg "1.2.3+algo-a" vs "1.2.3+algo-b"
  - OR can be done with hardware_version, but device has to have settings to opt-in to the correct
    payload, since the device requests the ota version
...
<!--
  ...
  - forwards/backwards compatibility in multi-component systems
  - "combined package" version for Release Version + reported version (or attributes for subcomponent sw versions)
 -->
...
<!--
  - use a targeted cohort, like `beta` or `internal`, to push OTA to before distributing to production devices
  - use the "staged release" feature to roll out to a percentage of devices,
    in steps: for example, 5% -> 25% -> 50% -> 100%, with a delay between each step for verifying stability
  - especially for new devices, writing down an OTA playbook is a good idea (give example)
-->
