...
Note, the Memfault SDK **does not need** to be located within the `esp` directory, you may
place it anywhere.
...
The POST_BUILD command depends on `pyelftools` so you may need to install it by
...
# XXX Item 1 to add for Memfault
...
# XXX Item 2 to add for Memfault
...
### Add the Memfault Configuration Files
