...
`memfault_reboot_reason_get`. The MCU SDK uses this function to determine a
reboot reason from your platform's hardware. The MCU SDK implements
`memfault_reboot_reason_get` for several platforms, including:
