...
The `MEMFAULT_HEAP_STATS_*` macros extract the Link Register at the call site
where they're placed. If your `malloc`/`free` calls are being aggressively
inlined (eg if using LTO and/or high optimzation levels `-O3`), you might see
incorrect "Allocation site" values. Adding `__attribute__((noinline))` or
similar can help address that.
