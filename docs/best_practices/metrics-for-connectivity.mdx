...
For example, below is a diagram of many of the Bluetooth connection states that a device can be in. It would be wise to try and track
the amount of time that the device is in all of these states to see where devices are spending most of their time and likely power. 
...
We recommend tracking the following metrics to help diagnose connectivity issues:
