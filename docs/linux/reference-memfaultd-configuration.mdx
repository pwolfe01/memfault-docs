...
`memfaultd` can use different strategy to capture the memory of the program
which crashed.

By default, the `threads` strategy is used and will only capture the stacks of
all the threads in the program. This keeps the coredump to a small size and
provides most of the information that is useful for debugging.

You can limit the size of each stacks with `max_thread_size_kib`.
...
the memory regions included in the ELF core file generated by the kernel.
