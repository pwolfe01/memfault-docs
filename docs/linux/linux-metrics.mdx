...
expected to work equally well.

StatsD clients can be configured to communicate with CollectD using [the StatsD
plugin][collectd-plugin-statsd]. This will allow you to send application metrics
through to Memfault.
...
last line in your configuration**. This ensures each request contains the full
set of configured metrics.
...
You don't need to add a `LoadPlugin write_http` in your own config. One such
line is included in the generated configuration. Subsequent `LoadPlugin` lines
for the same plugin will have no effect.

### `Interval` and `FlushInterval`
...
Using `memfaultd`, you can control both settings at once using the
`collectd.interval_seconds` configuration option of `/etc/memfaultd.conf`. See
[a full configuration reference here][docs-configuration].
...
first experience with the Memfault platform.
