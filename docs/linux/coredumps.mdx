...
- `CONFIG_CORE_DUMP_DEFAULT_ELF_HEADERS=y` in order to enable default mappings.
  for processes. Specifically, Memfault recommends:
...
By default, the `threads` strategy is used and limits capture to all program
thread stacks. This reduces the coredump size but still keeping the most useful
...
If you would like to capture more memory, and include the heap and other memory
regions, you can use the `kernel_selection` strategy. This strategy captures all
the memory regions included in the ELF core file generated by the kernel (see
[`man core`][docs-man-core] core for kernel configuration options).
...
With this strategy, coredumps can grow as large as the amount of memory that the
program is using. If the uncompressed size exceeds
[coredump-max-size-kib][docs-coredump-max-size], the coredump will not be saved.

[coredump-max-size-kib]:
  /docs/linux/reference-memfaultd-configuration#coredump_max_size_kib
...
**Admin → Organization Auth Tokens**. This can only be done by an organization
administrator.
...
`memfault` requires `bitbake` to upload the symbols. It's recommended that you
source the Yocto build environment script using `source oe-init-build-env`
before running `upload-yocto-symbols`.

### Manual upload

Another option is to upload the symbol files individually using the Memfault Web
Application. The symbol files you upload here must be individual ELF files
containing both the program text as well as debugging symbols. You would
typically build them with `-g` and remove the debug symbols before distribution
with the `strip` command.

<p>
  Open <b>Software -> Symbol Files</b> and click on <b>Upload Symbol File</b>,
  or simply{" "}
...
The Yocto-generated debug image (for example in `.tar.bz2` format) contains
stripped binaries and (separately) debug binaries. To upload them to Memfault,
...
If you're not using Yocto, or if you are building some binaries outside of
Yocto, you can manually upload symbols using our [REST
