...
If you're using Yocto, the `meta-oe` layer includes [a recipe for `collectd`],
so you may be able to just add `collectd` to your dependencies, e.g. by
appending it to `IMAGE_INSTALL`.

[source-collectd-recipe]:
  https://github.com/openembedded/meta-openembedded/tree/master/meta-oe/recipes-extended/collectd

### Optional: a StatsD client for application metrics
