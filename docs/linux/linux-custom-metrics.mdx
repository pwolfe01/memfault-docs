...
simply configures the `write_http` plugin to talk to Memfault. Additionally, it
creates a [CollectD post-cache chain][collectd-chains] that points to the
configured `write_http` plugin, allowing other chains (provided by you) to exist
if needed.
