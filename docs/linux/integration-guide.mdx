...
`memfaultd` will work with or without systemd. When systemd is available, it
will automatically be used to detect normal reboots and shutdowns (refer to our
[reboot reason tracking guide][docs-linux-reboot]).
...
By default, `memfaultd` will use the [`rustls`][rustls-homepage] TLS library,
which is a full TLS implementation in Rust. We recommend this option for most
users.

In some scenario you may want to use OpenSSL:

- To avoid having two SSL implementations in the system and minimize the size of
  the final image.
- For architectures which are not supported by `rustls` (notably, `mips`
  variants).
...
[rustls-homepage]: https://github.com/rustls/rustls
