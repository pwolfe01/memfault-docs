...
When adding fluent-bit to your system, make sure it is started _after_
`memfaultd`. If fluent-bit is started before `memfaultd`, the logs that happened
before `memfaultd` started may be lost. To ensure the correct boot order using
systemd, add `After=memfaultd.service` to the fluent-bit service file.
