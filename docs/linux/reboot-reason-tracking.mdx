...
In this guide we will walk through how to use the reboot reason tracking feature
...
upload reboot events to the Memfault platform. This is enabled through the
`reboot` feature.
...
`reboot.last_reboot_reason_file` key in `memfaultd.conf`. Take a look at the
[`/etc/memfaultd.conf` reference][docs-configuration] for more information.
...
`upload_interval_seconds`][docs-configuration] for more information.
