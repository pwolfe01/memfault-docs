...
By default, periodic Bug Report collection is disabled, and all Caliper data
collection is enabled. This can be configured in the Memfault dashboard - see
...
`bort.properties`. This is supported from Bort 4.5.0 onwards.
...
Excluding the `com.memfault.intent.extra.PROJECT_KEY` parameter will reset to
the key compiled into the APK (from Bort 4.6.0).
...
the system (deprecated - see below).
...
and was completely removed in Bort 4.6.0.

:::

This configuration would upload individual files to Memfault. After the data is
collected from the system, it is uploaded up to 15 minutes later.

### Sync Behavior and Triggers

Memfault uses the Android JobScheduler APIs to schedule its work following
Android's best practices.

The "Allow Uploads on Metered Networks" and "Allow OTA downloads on Metered
Networks" settings available in your
[Project Settings - Data Sources](https://app.memfault.com/organizations/-/projects/-/settings/data-sources)
configure Memfault to put either a
[NetworkType.CONNECTED](https://developer.android.com/reference/androidx/work/NetworkType#CONNECTED)
(if true) or
[NetworkType.UNMETERED](https://developer.android.com/reference/androidx/work/NetworkType#UNMETERED)
constraint on its network requests.

Metrics, logs, and DropBoxManager Entries are uploaded every 2 hours by default.
OTAs are checked for every 12 hours. Both settings can be adjusted if desired by
contacting support.
